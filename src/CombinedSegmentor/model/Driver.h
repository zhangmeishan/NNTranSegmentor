...
#pragma omp parallel for schedule(static,1)
  ...
  void extractFeat(const std::vector<std::vector<string> >& sentences, const vector<vector<CAction> >& goldACs) {
    ...
    CAction answer;
    CStateItem* lastState;
    CStateItem* pGenerator;
    int step;
    for (int idx = 0; idx < num; idx++) {
      step = 0;
      state[step].clear();
      state[step].setInput(&sentences[idx]);
      lastState = &state[step];
      while (true) {
        lastState->prepare(&_hyperparams, &_modelparams, NULL);
        pGenerator = &state[step +1];
        answer.set(goldACs[idx][step]);
        lastState->move(pGenerator, answer);
        pGenerator->collectFeat(&_modelparams);
